...
function toKismetAlerts(entries) {
  const lines = entries
    .filter((e) => e.entry_type === 'IP')
    .map((e) => `alert=${e.value},${e.reason || 'Blacklisted by DurianDetector'}`);
  return ['# Kismet alert configuration — generated by DurianDetector', '# Add to kismet_alerts.conf', '', ...lines].join('\n');
}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {['PREMIUM', 'EXCLUSIVE'].includes(user?.tier?.toUpperCase()) && entries.length > 0 && (
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={() => exportFile(toKismetAlerts(entries), 'blacklist-kismet.conf')}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-xs text-slate-400 hover:text-white hover:bg-white/[0.06] transition-all cursor-pointer"
              >
                <Download className="w-3.5 h-3.5" />
                Kismet
